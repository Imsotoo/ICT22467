...
| หมวด | กฎ | รายละเอียด |
|------|----|------------|
| 👤 ผู้ใช้งาน | Role ที่มี | `admin` และ `owner` |
| | Default role | `admin` เมื่อสมัครใหม่ |
| 🏊 บ่อกุ้ง | สถานะบ่อ | `active`, `inactive`, `maintenance` |
| | รอบการเลี้ยง | บ่อหนึ่งมีได้หลายรอบ |
| | ลบบ่อ | ข้อมูลที่เกี่ยวข้องทั้งหมดถูกลบตาม (CASCADE) |
| 🦐 การเลี้ยงกุ้ง | ข้อมูลที่บันทึก | จำนวนลูกกุ้ง (ตัว) และวันที่เริ่มเลี้ยง |
| | สถานะ | `growing` และ `harvested` |
| 💧 คุณภาพน้ำ | วิธีบันทึก | เซนเซอร์ IoT หรือกรอกเอง (manual) |
| | device_id แบบ manual | `MANUAL-{pond_id}` |
| | ค่าที่วัด | อุณหภูมิ, pH, ออกซิเจน, ระดับน้ำ |
| 🍚 การให้อาหาร | หน่วยปริมาณอาหาร | กิโลกรัม |
| | เวลาบันทึก | ใช้เวลา server ณ ตอนนั้น (NOW()) |
| | ประเภทอาหาร | ระบุหรือไม่ระบุก็ได้ |
| 📈 การเจริญเติบโต | ค่าที่บันทึก | น้ำหนักเฉลี่ย (กรัม) และความยาวเฉลี่ย (ซม.) |
| | วันที่สุ่มตรวจ | ผู้ใช้เลือกเองได้ ไม่ใช้เวลา server |
